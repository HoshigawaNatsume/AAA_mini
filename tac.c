...
// lifangbo 
TAC *declare_var_array(char *name, char *size)
{
	SYM*t = mk_var(name);
	t->array_flag = 1;
	t->array_size = atoi(size);
	return mk_tac(TAC_VAR,t,NULL,NULL);
}

TAC *declare_var_pointer(char *name)
{
	SYM*t = mk_var(name);
	t->pointer_flag=1;
	return mk_tac(TAC_VAR,t,NULL,NULL);
...
TAC *declare_para_array(char *name)
{
	SYM*t=mk_var(name);
	t->array_flag = 1;
	t->array_address_flag = 1;
	t->formal_flag = 1;
	return mk_tac(TAC_FORMAL,t,NULL,NULL);
}
...
TAC *do_assign(SYM *var, EXP *exp)
{
	...
	var->array_offset_right = exp->array_offset;
	t->array_offset = var->array_offset;
	t->array_offset_right = var->array_offset_right;
	if(var->type !=SYM_VAR) error("assignment to non-variable");
	if(var->array_flag==1)
	{
		// printf("***%d,**%d",var->array_size,var->array_offset);
		code=mk_tac(TAC_COPY, var, exp->ret, t);
	}
	else{
...
TAC *do_call(char *name, EXP *arglist)
{
	...
	while(arglist !=NULL) /* Generate ARG instructions */
	{
		...
		temp->prev=code;
		temp->array_offset = arglist->array_offset;//
		code=temp;
...
TAC *do_lib(char *name, SYM *arg, int offset)
{
    TAC *a=mk_tac(TAC_ACTUAL, arg, NULL, NULL);
	//printf("***%d\n",arg->array_offset);
	a->array_offset = offset;
    TAC *c=mk_tac(TAC_CALL, NULL, (SYM *)strdup(name), NULL);
	// c->array_offset = arg->array_offset;
    c->prev=a;
    return c;
}
...
EXP *mk_exp_array(EXP *next, SYM *ret, TAC *code, int array_offset)
{
	EXP *exp=(EXP *)malloc(sizeof(EXP));
...
void tac_print(TAC *i)
{
	...
	switch(i->op)
	{
		...
		// 如果是数组
		if (i->a->array_flag==1){
			printf("%s[%d] = %s", to_str(i->a, sa), i->c->array_offset, to_str(i->b, sb));
		}
		else if(i->a->pointer_flag==1 && i->b->address_flag==0)
		{
			printf("*%s = %s", to_str(i->a, sa), to_str(i->b, sb));
		}
		else if(i->a->pointer_flag==1 && i->b->address_flag==1)
		{
			printf("%s = &%s", to_str(i->a, sa), to_str(i->b, sb));
		}
		else{
		...
		else if(i->a->array_flag==1 && i->a->array_address_flag!=1)//数组
		{	
			if (i->a->array_address_flag!=1){
				printf("actual %s[%d]", to_str(i->a, sa), i->array_offset);
			}
			else{
				printf("actual %s", to_str(i->a, sa));
			}
			// array_count--;
			//printf("actual %s[%d]", to_str(i->a, sa), array_number[array_count]);
			// printf("actual %s[%d]", to_str(i->a, sa), i->array_offset);
		...
		{	
			if (i->a->array_address_flag!=1){
				printf("formal %s[%d]", to_str(i->a, sa), i->array_offset);
			}
			else{
				printf("formal %s", to_str(i->a, sa));
			}
		...
		// 如果是数组
		if (i->a->array_flag==1){
			printf("var %s[%d]", to_str(i->a, sa), i->a->array_size);
		}
		else{
